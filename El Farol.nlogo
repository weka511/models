...
to go
  ...
  ;; advance the clock
  tick
    let attendance-ratio attendance / overcrowding-threshold
  set  sum-attendance sum-attendance + attendance-ratio
  set mean-attendance sum-attendance / ticks
  set  sum-attendance2 sum-attendance2 + attendance-ratio * attendance-ratio
  let variance-attendance sum-attendance2 / ticks - (sum-attendance / ticks) * (sum-attendance / ticks)
  if variance-attendance > 0 [set sigma-attendance sqrt(variance-attendance)]
end
...
<experiments>
  <experiment name="experiment" repetitions="1" runMetricsEveryStep="false">
    ...
    <metric>mean-attendance</metric>
    <metric>sigma-attendance</metric>
    <steppedValueSet variable="memory-size" first="1" step="1" last="20"/>
    <steppedValueSet variable="number-strategies" first="1" step="1" last="20"/>
